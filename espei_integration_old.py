...
def create_espei_dataset_from_mp_row(
    ...
    # Create ESPEI dataset for formation enthalpy (HM_FORM)
    # For a single composition point, we need one configuration with occupancies
    # The sublattice_configurations should be a list with one configuration
    # Each configuration is a list of lists (one per sublattice)
    ...
    # ESPEI dataset format for formation energy:
    # sublattice_configurations: list of configurations, each configuration is a list of sublattices
    #   For 1 sublattice with multiple species: [[[species1, species2, ...]]]
    # sublattice_occupancies: list of occupancy sets for each configuration
    #   For the occupancy of each species: [[[occ1, occ2, ...]]]
    # sublattice_site_ratios: [ratio] for number of sites on each sublattice
    #
    # Example for Fe-Cr liquid with single sublattice:
    #   configurations: [[['CR', 'FE']]] - one config, one sublattice, two species
    #   occupancies: [[[0.3, 0.7]]] - occupancy for CR and FE on that sublattice
    #   site_ratios: [1.0] - one sublattice with ratio 1.0

    dataset = {
        "components": sorted_elements,
        "phases": ["LIQUID"],
        "solver": {
            "mode": "manual",
            "sublattice_site_ratios": [1.0],
            "sublattice_configurations": [[sorted_elements]],  # [[all elements]] - one config, one sublattice
            "sublattice_occupancies": [[occupancies]]  # [[occupancies]] - matching structure
        },
        "conditions": {
            "P": 101325,
            "T": temperature
        ...
        "comment": f"Formation energy data from Materials Project for {row.get('formula', 'unknown')}"
    }

    return dataset


def create_phase_models_json(elements: List[str], output_path: Path) -> Path:
    """
    Create a basic phase models JSON file for ESPEI.
    ...
    """
    # Create a simple liquid phase model
    # In a real application, you'd want more sophisticated phase models
    phase_models = {
        "components": sorted(elements),
        "phases": {
            "LIQUID": {
                "sublattice_model": [sorted(elements)],
                "sublattice_site_ratios": [1.0]
            }
        }
    }
...
def convert_mp_csv_to_espei_datasets(
    ...
    -------
    tuple
        (num_datasets_created, list_of_elements, datasets_dir, element_column_map)
    """
    # Load CSV
    ...
    for idx, row in df.iterrows():
        ...
        dataset = create_espei_dataset_from_mp_row(row_modified, elements, temperature=temperature)

        if dataset is not None:
            # Save each dataset as a separate JSON file
            mat_id = row.get('material_id', f'material_{idx}')
            filename = f"{mat_id.replace('-', '_')}.json"
    ...
    return datasets_created, elements, datasets_dir, element_columns
